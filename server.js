...
const { sendClimateReportEmail, sendSmartFarmingReportEmail } = require('./emailReport');
  ...
  try {
    ...
    if (transaction.success === true || transaction.success === 'true') {
      ...
      try {
        ...
        const updated = reportsStore.updateReport(reportId, { status: 'ready', reportData });

        // نرسل نسخة أرشيفية لبريد الشركة تلقائياً — لا نُفشل الطلب لو تعذّر الإرسال
        // (الفلاح يجب أن يستلم تقريره بغض النظر عن نجاح/فشل الأرشفة البريدية)
        sendSmartFarmingReportEmail(updated).catch((mailErr) => {
          console.error('[بريد] فشل إرسال نسخة أرشيفية لتقرير الزراعة الذكية:', mailErr.message);
        });
      } catch (genErr) {
...
// ===========================================================================
// 5) POST /api/send-report-email — زر "إرسال التقرير لبريد الشركة" في حاسبة
// أصول المناخ (index.html). البيانات مُدخلة يدوياً من العميل عبر الواجهة —
// التقرير الناتج إرشادي وليس شهادة كربون معتمدة (موضّح داخل نص الإيميل نفسه).
// body: { owner, location, areaFeddan, ndvi, emissionsTons, absorbedTons,
//         netCarbonTons, incomeMin, incomeMax, snapshotImage? }
// ===========================================================================
app.post('/api/send-report-email', async (req, res) => {
  try {
    const { snapshotImage, ...reportData } = req.body || {};

    if (!reportData.owner && !reportData.areaFeddan) {
      return res.status(400).json({ error: 'بيانات التقرير ناقصة — لا يوجد شيء لإرساله' });
    }

    await sendClimateReportEmail(reportData, snapshotImage);
    res.json({ success: true });
  } catch (err) {
    console.error('send-report-email error:', err.message);
    res.status(500).json({ error: 'تعذر إرسال التقرير لبريد الشركة — تأكد من إعداد SMTP في .env', detail: err.message });
  }
});
