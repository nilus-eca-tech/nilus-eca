...
## إعداد إرسال التقارير بالبريد (SMTP)

كل التقارير — تقرير حاسبة أصول المناخ (زر "إرسال التقرير لبريد الشركة") ونسخة أرشيفية تلقائية
من كل تقرير زراعة ذكية مدفوع — تُرسل إلى `COMPANY_REPORT_EMAIL` (افتراضياً `waadnasr144@gmail.com`).

1. في `.env`: اضبط `COMPANY_REPORT_EMAIL` لو أردت تغيير العنوان المستقبِل
2. اضبط `SMTP_USER` و`SMTP_PASS` — الحساب الذي *يرسل* الإيميلات فعلياً:
   - **الأفضل**: بريد مخصص للشركة (مثلاً `reports@nilus-eca.com`) عبر مزوّد كـ Google Workspace أو SendGrid/Mailgun
   - **بديل سريع للتجربة**: بريد Gmail شخصي — لكن يتطلب "App Password" مخصص وليس كلمة المرور العادية،
     يُنشأ من `myaccount.google.com/apppasswords` بعد تفعيل التحقق بخطوتين
3. لا تضع بريد الشركة `waadnasr144@gmail.com` نفسه في `SMTP_USER` إلا لو كنت ستولّد له App Password —
   الأنسب أن يكون `SMTP_USER` بريد إرسال منفصل و`COMPANY_REPORT_EMAIL` هو بريد الاستلام فقط

⚠️ **مهم**: تقرير حاسبة أصول المناخ مبني على بيانات يُدخلها العميل يدوياً في المتصفح (ديزل، يوريا،
عدد أشجار...) — وهو **تقرير إرشادي غير معتمد**، وليس شهادة كربون رسمية. هذا موضّح صراحة في نص
الإيميل نفسه لتفادي أي التباس تنظيمي أو قانوني.
